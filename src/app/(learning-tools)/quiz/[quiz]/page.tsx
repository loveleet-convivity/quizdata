...
import { useGetQuizQuestionsQuery } from "@/app/state/slices/apiSlice";
import { useDispatch, useSelector } from "react-redux";
import { quizSelector, setter } from "@/app/state/slices/quizSlice";
import Equation from "./_components/questiontypes/Equation";
export default function Home() {
  ...
  const { data, error, isLoading, isFetching } =
    useGetQuizQuestionsQuery(64);
  const dispatch = useDispatch();
  const { questionlist } = useSelector(quizSelector);

  // console.log(data?.quiz_questions, ">>>questions");
  ...
  useEffect(() => {
    if (data?.quiz_questions) {
      const formattedQuestions = data.quiz_questions.map((question: any) => ({
        id: question?.data._id,
        title: question.data._title,
        question: question.data._question,
        answer: question.data._answerData,
        type: question.data._answerType,
      }));
      dispatch(setter(formattedQuestions));

      if (formattedQuestions.length > 0) {
        setCurrentQuestion(formattedQuestions[0].id);
      }
    }
  }, [data]);
    ...
    <div className="flex bg-[#F7F5F1] max-md:h-[100vh] w-full relative container max-w-[90rem] mx-auto px-0">
      <aside className="flex max-md:flex-col-reverse h-[100vh]">
        <div className="max-md:absolute max-md:top-10">
          {isSidebarVisible && (
            <QuizSidebar btn={btnheight} setter={setCurrentQuestion}>
              <SidebarHead />
              <SidebarNavigation />
            </QuizSidebar>
          )}
      ...
      <Question show={isSidebarVisible} btn={btnheight}>
        ...
        <QuestionBox>
          <QuestionContent data={currentQuestion} setter={setCurrentQuestion} />
          {/* <Equation data={currentQuestion} /> */}
          <QuestionBoxTools
